...
class OrangeGrove
  ...
  def to_s
    return "#{size} trees out of a maximum of #{@capacity}, with soil quality #{@soil_quality * 100}%."
  end
  ...
  def plant_trees(n=1)
    ...
    if size == @capacity
      ...
      n = @capacity - size
      @soil_quality = 0
      puts "After planting #{n} more trees (grove is now at capacity):"
    else
  ...
  class OrangeTree
    ...
    def to_s
      return "Tree is a #{@age}-year-old #{@age_category} tree, #{display_height} tall with #{@orange_count} oranges."
    end
